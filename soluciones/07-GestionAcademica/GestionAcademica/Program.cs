...
void Main() {
    ...
    foreach (var p in PersonasFactory.Seed())
        try {
            ...
            Log.Warning($"⚠️ Semilla ignorada ({p.Dni}): {ex.Message}");
    ...
    Console.WriteLine("🚀 SISTEMA DE GESTIÓN ACADÉMICA (ESTILO DAW)");
    ...
    do {
        ...
        switch (opcion) {
            ...
            case OpcionMenu.Salir: Console.WriteLine("\n👋 Cerrando el sistema. ¡Hasta pronto!"); break;
        }
...
void MostrarMenu() {
    ...
    Console.WriteLine(new string('─', 45));
    Console.WriteLine($"  {(int)OpcionMenu.ListarTodas}. 👥 Listar todo el personal");
    Console.WriteLine($"  {(int)OpcionMenu.BuscarDni}. 🔍 Buscar persona por DNI");
    Console.WriteLine($"  {(int)OpcionMenu.BuscarId}. 🆔 Buscar persona por ID");

    Console.WriteLine("\n🎓 --- 2. GESTIÓN DE ESTUDIANTES ---");
    Console.WriteLine(new string('─', 45));
    Console.WriteLine($"  {(int)OpcionMenu.ListarEstudiantes}. 📜 Listar Estudiantes");
    Console.WriteLine($"  {(int)OpcionMenu.AnadirEstudiante}. ➕ Añadir Estudiante");
    Console.WriteLine($"  {(int)OpcionMenu.ActualizarEstudiante}. 📝 Actualizar Estudiante");
    Console.WriteLine($"  {(int)OpcionMenu.EliminarEstudiante}. 🗑️  Eliminar Estudiante");
    Console.WriteLine($"  {(int)OpcionMenu.InformeEstudiantes}. 📊 Informe de Rendimiento");

    Console.WriteLine("\n👨‍🏫 --- 3. GESTIÓN DE DOCENTES ---");
    Console.WriteLine(new string('─', 45));
    Console.WriteLine($"  {(int)OpcionMenu.ListarDocentes}. 📜 Listar Docentes");
    Console.WriteLine($"  {(int)OpcionMenu.AnadirDocente}. ➕ Añadir Docente");
    Console.WriteLine($"  {(int)OpcionMenu.ActualizarDocente}. 📝 Actualizar Docente");
    Console.WriteLine($"  {(int)OpcionMenu.EliminarDocente}. 🗑️  Eliminar Docente");
    Console.WriteLine($"  {(int)OpcionMenu.InformeDocentes}. 📈 Informe de Experiencia");
...
void ListarTodas(IPersonasService service) {
    ...
    Console.WriteLine("⚙️  Criterios: 1.ID, 2.DNI, 3.Apellidos, 4.Nombre, 5.Ciclo");
...
void ListarEstudiantes(IPersonasService service) {
    ...
    Console.WriteLine("⚙️  Criterios: 1.ID, 2.DNI, 3.Apellidos, 4.Nombre, 5.Nota, 6.Curso, 7.Ciclo");
...
void AnadirNuevoEstudiante(IPersonasService service) {
    ...
    if (PedirConfirmacion("¿Confirmar alta?"))
        try {
            ...
            Console.WriteLine($"❌ CONFLICTO: {ex.Message}");
        }
}
...
void ActualizarEstudiante(IPersonasService service) {
    ...
    try {
        ...
        if (p is not Estudiante est) {
            Console.WriteLine("❌ ERROR: No es un Estudiante.");
            return;
        }
        ...
        if (PedirConfirmacion("¿Actualizar?")) {
            var actualizado = service.Update(act);
...
void MostrarInformeEstudiantes(IPersonasService service) {
    ...
    Console.WriteLine("⚙️  Alcance: 1.Global, 2.Por Ciclo, 3.Por Curso, 4.Clase Específica");
    ...
    Console.WriteLine($"📍 ALCANCE: {desc}");
    Console.WriteLine(
...
void ListarDocentes(IPersonasService service) {
    Console.WriteLine("\n👨‍🏫 --- LISTADO DE DOCENTES ---");
    Console.WriteLine("⚙️  Criterios: 1.ID, 2.DNI, 3.Apellidos, 4.Nombre, 5.Experiencia, 6.Módulo, 7.Ciclo");
    var op = LeerCadenaValidada("🎯 Seleccione criterio: ", "^[1-7]$", "Elija entre 1 y 7.");
...
void ActualizarDocente(IPersonasService service) {
    ...
    try {
        ...
        if (p is not Docente doc) {
            Console.WriteLine("❌ ERROR: No es un Docente.");
            return;
        }
...
void MostrarInformeDocentes(IPersonasService service) {
    ...
    Console.WriteLine("⚙️  Alcance: 1.Global, 2.Por Ciclo");
...
void ImprimirTablaPersonas(ILista<Persona> lista) {
    ...
    foreach (var p in lista) {
        ...
        Console.WriteLine($" {p.Id,-5} | {p.Dni,-10} | {p.NombreCompleto,-35} | {ciclo,-8} | {tipo}");
    }

    Console.WriteLine(line);
}

void ImprimirTablaEstudiantes(ILista<Estudiante> lista) {
...
void ImprimirTablaDocentes(ILista<Docente> lista) {
...
void ImprimirFichaPersona(Persona p) {
    ...
    Console.WriteLine();
    Console.WriteLine(line);
    Console.WriteLine("  🆔 IDENTIDAD ACADÉMICA");
    Console.WriteLine(line.Replace('━', '─'));
    Console.WriteLine($"  🆔 ID:          {(p.Id == 0 ? "PENDIENTE" : p.Id)}");
    ...
    if (p is Estudiante e) {
        ...
        Console.WriteLine($"  📝 NOTA:        {e.Calificacion.ToString("F2", Configuracion.Locale)}");
        Console.WriteLine($"  🎖️  EVAL:        {e.CalificacionCualitativa}");
        Console.WriteLine($"  📂 CICLO:       {e.Ciclo}");
        Console.WriteLine($"  📅 CURSO:       {e.Curso}");
    }
    else if (p is Docente d) {
        Console.WriteLine("  🎭 TIPO:        👨‍🏫 DOCENTE");
        Console.WriteLine($"  ⏳ EXP:         {d.Experiencia} años");
        Console.WriteLine($"  📚 MOD:         {d.Especialidad}");
        Console.WriteLine($"  📂 CICLO:       {d.Ciclo}");
    }

    if (p.CreatedAt != default) {
        Console.WriteLine(new string('─', 65));
        Console.WriteLine($"  📅 ALTA (LOC):  {p.CreatedAt.ToLocalTime().ToString("g", Configuracion.Locale)}");
        Console.WriteLine($"  🔄 MOD  (LOC):  {p.UpdatedAt.ToLocalTime().ToString("g", Configuracion.Locale)}");
...
void ImprimirErroresValidacion(ILista<string> errores) {
...
string LeerDniValidado() {
    while (true) {
        ...
        var d = Console.ReadLine()?.Trim().ToUpper() ?? "";
        if (ValidarDniCompleto(d)) return d;
        Console.WriteLine("❌ ERROR: DNI inválido.");
    }
}
...
Ciclo LeerCiclo() {
    Console.WriteLine("📂 Ciclos Disponibles: 1.DAM, 2.DAW, 3.ASIR");
    return (Ciclo)(int.Parse(LeerCadenaValidada("🎯 Elija Ciclo: ", @"^[1-3]$", "Seleccione entre 1 y 3.")) - 1);
}

Curso LeerCurso() {
    Console.WriteLine("📅 Cursos Disponibles: 1.Primero, 2.Segundo");
