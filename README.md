...
- [Programación - 06 Programación con Genéricos. Programación Funcional. Uso de TDAs y Colecciones](#programación---06-programación-con-genéricos-programación-funcional-uso-de-tdas-y-colecciones)
  ...
  - [3. Programación Funcional en C#](#3-programación-funcional-en-c)
    ...
    - [3.8. Pattern Matching](#38-pattern-matching)
      - [3.8.1. Fundamentos y Motivación](#381-fundamentos-y-motivación)
      - [3.8.2. Tipos de Patterns](#382-tipos-de-patterns)
        - [3.8.2.1. Type Pattern - Verificación de Tipo](#3821-type-pattern---verificación-de-tipo)
        - [3.8.2.2.  Constant Pattern - Valores Constantes](#3822--constant-pattern---valores-constantes)
        - [3.8.2.3. Relational Pattern - Comparaciones](#3823-relational-pattern---comparaciones)
        - [3.8.2.4. Property Pattern - Propiedades de Objetos](#3824-property-pattern---propiedades-de-objetos)
        - [3.8.2.5. Positional Pattern - Deconstrucción](#3825-positional-pattern---deconstrucción)
        - [3.8.2.6. Discard Pattern - Ignorar Valores](#3826-discard-pattern---ignorar-valores)
        - [3.8.2.7. Control de Nulos](#3827-control-de-nulos)
      - [3.8.3. Combinación de Patterns](#383-combinación-de-patterns)
      - [3.8.4. Switch Expression vs Switch Statement](#384-switch-expression-vs-switch-statement)
      - [3.8.5. Casos de Uso Prácticos](#385-casos-de-uso-prácticos)
      - [3.8.3. Resumen](#383-resumen)
...
- [Podcast](https://youtu.be/JIIgzemG1-M)
- [Resumen](https://youtu.be/vD8irjEweE4)
- [Programación con Genéricos](https://youtu.be/jlKBynUhFAA)
- [Programación Funcional](https://youtu.be/l8qXIwkQ5PQ) 
- [Listas](https://youtu.be/GrYYbUXUVa4)
- [Diccionarios](https://youtu.be/2fWYjfa2rPw)
- [Conjuntos](https://youtu.be/qaurOuoh0a0)
- [Pilas y Colas](https://youtu.be/GIYfC8ZlKyI)
...
Tienes razón, me extendí demasiado.  Aquí tienes una versión **más concisa y centrada** sobre **Pattern Matching en C#** que se ajusta mejor al nivel y formato de tus apuntes:

---

### 3.8. Pattern Matching

**Pattern Matching** permite inspeccionar y descomponer valores de forma declarativa, reemplazando múltiples `if-else` o `switch` tradicionales por código más legible y expresivo.

#### 3.8.1. Fundamentos y Motivación
...
#### 3.8.2. Tipos de Patterns

##### 3.8.2.1. Type Pattern - Verificación de Tipo
...
##### 3.8.2.2.  Constant Pattern - Valores Constantes
...
##### 3.8.2.3. Relational Pattern - Comparaciones
...
##### 3.8.2.4. Property Pattern - Propiedades de Objetos
...
string descripcion = p switch
{
    ...
    { Edad:  >= 18, Nombre: "Ana" } => "Ana es mayor de edad",
    { Edad: >= 65 } => "Senior",
    _ => "Adulto"
};
```

##### 3.8.2.5. Positional Pattern - Deconstrucción
...
##### 3.8.2.6. Discard Pattern - Ignorar Valores

```csharp
object obj = 42;

// _ ignora el valor específico
bool esNumero = obj switch
{
    int _ => true,
    _ => false
};
...
if (texto is {})
{
    Console.WriteLine($"Texto: {texto. ToUpper()}");
}
...
string resultado = texto switch
{
    null => "Sin valor",
    "" => "Vacío",
    {} => $"Valor: {texto}" // Solo si NO es null
};
```

#### 3.8.3. Combinación de Patterns

**Logical Patterns (`and`, `or`, `not`)**:
```csharp
int numero = 25;

bool esValido = numero is > 0 and < 100;  // Entre 0 y 100
bool esExtremo = numero is < 10 or > 90;  // Menor que 10 o mayor que 90
bool noEsCero = numero is not 0;           // Diferente de 0
```
...
Producto producto = new Producto("Laptop", 1200, 5);

string estado = producto switch
{
    { Stock: 0 } => "Agotado",
    { Stock:  > 0 and <= 5 } => "Stock bajo",
    { Precio: >= 1000 } when producto.Stock > 10 => "Premium disponible",
    _ => "Disponible"
};
```

#### 3.8.4. Switch Expression vs Switch Statement
...
#### 3.8.5. Casos de Uso Prácticos

**Procesamiento de diferentes tipos:**
```csharp
public static string ProcesarMensaje(object mensaje) => mensaje switch
{
    string texto => $"Texto: {texto}",
    int numero => $"Número: {numero}",
    bool bandera => bandera ? "Verdadero" : "Falso",
    null => "Mensaje nulo",
    _ => "Tipo desconocido"
};
```
...
```csharp
public static string ClasificarNota(double nota) => nota switch
{
    >= 9 => "Sobresaliente",
    >= 7 and < 9 => "Notable",
    >= 5 and < 7 => "Aprobado",
...
```csharp
public record Pedido(int Id, string Estado, decimal Total);

public static string DescribirPedido(Pedido pedido) => pedido switch
{
    { Estado: "Pendiente", Total:  < 50 } => "Pedido pequeño pendiente",
    { Estado: "Enviado", Total: >= 100 } => "Pedido grande en camino",
    { Estado: "Entregado" } => "Pedido completado",
...
| Pattern             | Uso                            | Ejemplo                       |
| ------------------- | ------------------------------ | ----------------------------- |
| **Type**            | Verificar tipo y extraer valor | `obj is string texto`         |
| **Constant**        | Comparar con valor exacto      | `x is 42`                     |
| **Relational**      | Comparaciones (`<`, `>`, etc.) | `edad is >= 18`               |
| **Property**        | Propiedades de objetos         | `{ Edad: > 18 }`              |
| **Positional**      | Deconstrucción                 | `(0, 0) => "Origen"`          |
| **Logical**         | `and`, `or`, `not`             | `x is > 0 and < 100`          |
| **Guard (`when`)**  | Condición adicional            | `case > 0 when x % 2 == 0 =>` |
| **Discard (`_`)**   | Ignorar valor                  | `_ => "Default"`              |
| **Not null (`{}`)** | Control de nulos               | `if (obj is {}) { ... }`      |

**Ventajas:**
- ✓ Código más declarativo y legible
- ✓ Menos errores de casting
- ✓ Exhaustividad verificable (el compilador avisa si falta un caso)
- ✓ Mejor integración con tipos modernos (records, tuples)
...
```mermaid
graph TD
    %% 1. Definición de Nodos con Estilos Inline
    %% Interfaces (Azul)
    Enumerable["IEnumerable&lt;T&gt;"]
    Collection["ICollection&lt;T&gt;"]
    ListInt["IList&lt;T&gt;"]
    DictInt["IDictionary&lt;K, V&gt;"]
    SetInt["ISet&lt;T&gt;"]

    %% Clases Concretas (Naranja)
    ListClass["List&lt;T&gt;"]
    DictClass["Dictionary&lt;K, V&gt;"]
    SetClass["HashSet&lt;T&gt;"]
    ...
    %% 3. Aplicación de Colores (classDef)
    classDef interfaz fill:#E1F5FE,stroke:#01579B,stroke-width:2px,color:#01579B
    classDef clase fill:#FFF3E0,stroke:#E65100,stroke-width:1px,color:#E65100

    class Enumerable,Collection,ListInt,DictInt,SetInt interfaz
    class ListClass,DictClass,SetClass clase
```
...
graph TD
    ...
    classDef pregunta fill:#fff4dd,stroke:#d4a017,stroke-width:2px;
    classDef resultado fill:#e1f5fe,stroke:#01579b,stroke-width:2px;
    classDef concurrente fill:#f3e5f5,stroke:#4a148c,stroke-width:1px;

    Start((INICIO)):::inicio --> P1{¿Buscar por CLAVE?}:::pregunta

    %% Rama Diccionarios
    P1 -- SÍ --> P2{¿Importa el ORDEN?}:::pregunta
    P2 -- SÍ --> P3{¿Orden automático?}:::pregunta
    P3 -- SÍ --> SD[SortedDictionary]:::resultado
    P3 -- NO --> OD[OrderedDictionary]:::resultado
    P2 -- NO --> D[Dictionary]:::resultado

    %% Rama No Diccionarios
    P1 -- NO --> P4{¿Permitir REPETIDOS?}:::pregunta
    
    %% Subrama Sets
    P4 -- NO --> P5{¿Ordenados?}:::pregunta
    P5 -- SÍ --> SS[SortedSet]:::resultado
    P5 -- NO --> HS[HashSet]:::resultado

    %% Subrama Listas y Otros
    P4 -- SÍ --> P6{¿Tipo de Acceso?}:::pregunta
    P6 -- FIFO --> Q[Queue]:::resultado
    P6 -- LIFO --> ST[Stack]:::resultado
    P6 -- Por Índice --> P7{¿Tamaño Fijo?}:::pregunta
    P7 -- SÍ --> AR[Array]:::resultado
    P7 -- NO --> L[List]:::resultado
    P6 -- Extremos --> LL[LinkedList]:::resultado

    %% Requisitos Especiales (Conexiones laterales para no ensuciar el flujo)
    D & L & HS --- P8{¿Multihilo?}:::pregunta
    P8 -- SÍ --> Conc[<b>Colecciones Concurrentes</b><br/>ConcurrentDictionary<br/>ConcurrentQueue<br/>ConcurrentStack<br/>ConcurrentBag]:::concurrente
    
    L & SS & HS --- P9{¿Inmutable?}:::pregunta
    P9 -- SÍ --> Imm[<b>System.Collections.Immutable</b><br/>ImmutableList<br/>ImmutableArray<br/>ImmutableHashSet<br/>...]:::concurrente

    %% Estilos de nodos
    class SD,OD,D,SS,HS,Q,ST,AR,L,LL resultado
```
